...
//(bi)
val counts1 = text.map(r => r.getString(0).replaceAll("[^a-zA-Z0-9 ]", " ").toLowerCase()).flatMap(_.split("\\s+"))
val counts2 = counts1.filter(x => x.length > 5).filter(x => x.length < 25)
val dataArray = counts2.collect()

dataframe.withColumn("words", typedLit(dataArray))
dataframe.show()
